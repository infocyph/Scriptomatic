...
if [ -r "$ROOTCA" ]; then
  ...
  if [ -n "$src_hash" ] && [ "$src_hash" != "$dst_hash" ] && command -v update-ca-certificates >/dev/null 2>&1; then
    if command -v sudo >/dev/null 2>&1; then
      sudo install -m 0644 "$ROOTCA" "$ROOTCA_DEST" 2>/dev/null || true
      sudo update-ca-certificates >/dev/null 2>&1 || true
    else
      install -m 0644 "$ROOTCA" "$ROOTCA_DEST" 2>/dev/null || true
...
install_deps() {
  ...
  if [ -f package-lock.json ]; then
    # shellcheck disable=SC2086 # historical fixed npm flags.
